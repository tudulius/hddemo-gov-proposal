...
if 'groq_models' not in st.session_state:
    st.session_state.groq_models = ["deepseek-r1-distill-qwen-32b"]
if 'selected_groq_model' not in st.session_state:
    st.session_state.selected_groq_model = "deepseek-r1-distill-qwen-32b"
...
with st.sidebar:
    ...
    # GROQ API 키 및 모델 설정
    groq_key = st.text_input("GROQ API 키를 입력하세요",
                            type="password",
    ...
    if groq_key:
        ...
        try:
            ...
            st.error(f"GROQ API 키 검증 실패: {str(e)}")

# 메인 페이지
st.title("정부지원사업 사업계획서 작성")

# API 키 확인
if not (st.session_state.openai_api_key or st.session_state.claude_api_key or st.session_state.groq_api_key):
        ...
        with st.spinner("사업계획서를 작성중입니다..."):
            try:
                ...
                if st.session_state.openai_api_key:
                    ...
                    groq = Groq(api_key=st.session_state.groq_api_key)
                    
                    # 데이터프레임을 청크로 나누기
                    chunk_size = 1000  # 한 청크당 약 1000자
                    df_text = st.session_state.announcement_df.to_string()
                    ...
                    for i in range(num_chunks):
                        ...
                        chunk_text = df_text[start_idx:end_idx]
                        
                        # 진행 상황 표시
                        st.write(f"분석 진행 중... ({i+1}/{num_chunks})")
                        
                        chunk_prompt = f"""이전까지의 분석 결과: {full_response if full_response else '아직 없음'}

현재 분석할 공고 내용 부분:
{chunk_text}

이 부분에서 발견되는 추가적인 주요 항목이나 기존 항목에 대한 보완 사항을 분석해주세요."""
                        ...
                        try:
                            ...
                            for chunk in completion:
                                if chunk.choices[0].delta.content is not None:
                                    ...
                                    full_response = full_response + chunk_response
                                    response_container.markdown(full_response)
                            
                        except Exception as e:
                            st.error(f"청크 {i+1} 처리 중 오류 발생: {str(e)}")
                            continue
